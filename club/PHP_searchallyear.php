...
if (!$conn) {
  ...
  $clubVal2 = "clubID='$type' "
	...
	$sql = "select president, member.year from execteam, club, member where club.clubID = execteam.clubID AND president = member.name" .$clubVal;           #show all presidents
  $sql = "select viceP, member.year from execteam, club, member where club.clubID = execteam.clubID and viceP = member.name" .$clubVal;                 #show all vice presidents
  $sql = "select treasurer, member.year from execteam, club, member where club.clubID = execteam.clubID and treasurer = member.name" .$clubVal;          # show all treasurer
  $sql = "select riskM, member.year from execteam, club, member where club.clubID = execteam.clubID and riskM = member.name" .$clubVal;                # show all risk managers
  $sql = "select advisor from execteam, club where club.clubID = execteam.clubID" .$clubVal;                                                            # show all advisors
  $sql = "select * from events where" .$clubVal2;                                                       # show events
  $sql = "select * from events where food = 1;" .$clubVal;                                                       # show events
  $sql = "select year, max(amountTotal) from budget where" .$clubVal2 " group by year";                                                             # show year with lowest budget
